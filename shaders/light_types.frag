...
vec4 directionalLight() {
	...
	// Light intensity is applied only to diffuse and specular component to manipulate base colors and reflectivity:
	return (texture(tex0, texCoord) * (diffuse + ambient) + texture(tex1, texCoord).r * specular) * lightColor;  // Specular map has only the red channel here. Therefore ".r" used.
}
...
vec4 spotLight() {
	...
	// Light intensity is applied only to diffuse and specular component to manipulate base colors and reflectivity:
	return (texture(tex0, texCoord) * (diffuse * intensity + ambient) + texture(tex1, texCoord).r * specular * intensity) * lightColor;  // Specular map has only the red channel here. Therefore ".r" used.
}
